    ...
    executeRx(RxBus.getInstance().registerRxObserver(pair -> {
      ...
      switch (pair.first) {
        case EVENT_POST_REFRESH:
          getPostList(0, null, receivedData -> postItemList.setValue(receivedData.getData()),
              errorData -> {
              });
          break;
...
//              }
  ...
  public void getPostList(int page, String search, SuccessCallback<ResponseModel<ArrayList<PostModel>>> callback, ErrorCallback errorCallback) {
    executeRx(PostApi.getInstance().getPostList(page, search, callback, errorCallback));
  }
